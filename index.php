	...
	if($ordercount == 0){
		...
		foreach($orderItems as $o){
			...
			//acceptance_decision_date
				...
				//Log
				echo " Last Update: " . $orderData["last_updated_date"]->format("Y-m-d\TH:i:s")." Now: ".date('Y-m-d\TH:i:s', strtotime("now"))." DIFF: ".$diff_minutes. "<br><br>";
				...
				if($diff_minutes < 0){
					...
					foreach($orderLines->getItems() as $ol){	
						...
						$phone = '';
						if(isset($orderCustomer["phone"]) && $orderCustomer["phone"] != ""){
							$phone = $orderCustomer["phone"];
						}
						else if(isset($orderCustomer["phone_secondary"]) != null && $orderCustomer["phone_secondary"] != ""){
							$phone = $orderCustomer["phone_secondary"];
						}
